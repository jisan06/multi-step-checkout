    ...
    if (!empty($active_shipping_methods)) {
        foreach ($active_shipping_methods as $method) {
            ...
            <div class="shipping-fields" data-method-id="<?php echo esc_attr($method['id']); ?>" style="display: none;">
                ...
                <?php }else { ?>
                    <p class="form-row form-row-wide" id="calc_shipping_country_field">
                        <label for="calc_shipping_country"><?php esc_html_e( '地區', 'woocommerce' ); ?></label>
                        <select name="calc_shipping_country" id="calc_shipping_country" class="country_to_state country_select" rel="calc_shipping_state">
                            <option value="default"><?php esc_html_e( '請選擇地區', 'woocommerce' ); ?></option>
                            <?php
                    ...
                    <p class="delivery-address">
                        ...
                        <input type="text" placeholder="請輸入收貨地址" class="shipping-address" />
                    </p>
                    <p class="receipt-date">
                        <label>
                            收貨日期
                        </label>
                        <input type="datetime-local" placeholder="請選擇收貨日期" class="shipping-receipt-date date-time" />
                    </p>
                    <p class="delivery-time">
                        <label>
                            收貨時段
                        </label>
                        <input type="datetime-local" placeholder="請選擇收貨時段" class="shipping-time date-time" />
                    </p>
                    <p class="contact-number">
                        <label>
                            聯絡電話
                        </label>
                        <input type="number" placeholder="請輸入聯絡電話" class="shipping-number" />
                    </p>
                <?php } ?>
            </div>
        <?php } } ?>
